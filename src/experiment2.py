...
class SpineModel(nn.Module):
    ...
    def forward(self, x, coord):
        ...
        # 🔥 coord embedding
        coord_feat = self.coord_mlp(coord)  # (B,32)
        coord_feat = coord_feat.unsqueeze(1).repeat(1, S, 1)  # (B,S,32)

        # 🔥 attention input
        attn_input = torch.cat([lstm_out, coord_feat], dim=-1) # (B,S,1056)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        if auc > best_auc:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(SAVE_DIR,"best_model_convnext_coord.pth")
            )
