# attention_fusion_model.py (수정 버전)
import os
import numpy as np
import pandas as pd
import torch
...
from model_attention_variants import AttentionClassifier_Seg, AttentionClassifier_Heat, AttentionClassifier_Fusion
...
# 데이터 경로 설정
full_df = pd.read_csv("slice_level_labeled.csv")
image_dir = "rsna_2024_spine/train_images"
heatmap_dir = "rsna_2024_spine/heatmaps"
coord_dir = "rsna_2024_spine/heatmaps"
...
# 테스트할 attention 방식 선택
model_type = "fusion"  # "seg", "heat", "fusion"
if model_type == "seg":
    model = AttentionClassifier_Seg().cuda()
elif model_type == "heat":
    model = AttentionClassifier_Heat().cuda()
else:
    model = AttentionClassifier_Fusion().cuda()
...
for epoch in range(50):
    ...
    if auc > best_auc:
        best_auc = auc
        torch.save(model.state_dict(), os.path.join(save_dir, f"best_{model_type}_attention_model.pth"))
        patience_counter = 0
    else:
        patience_counter += 1
